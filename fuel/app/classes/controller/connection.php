...
class Controller_Connection extends Controller_Template {
    ...
    public function action_register() {
        ...
        if (Input::method() == 'POST' && Security::check_token()) {
            ...
            if (!empty($user_id))
                $user_data = Model_User::find($user_id);
            else
                $user_data = false;
            ...
            if ($error_count != 0) {
                if ($error_count == 1) {
                    $result_message = 'Radusies viena kļūda: <br/><ul>' . $error_message . "</ul>";
                } else {
                    $result_message = 'Radušās ' . $error_count . ' kļūdas: <br/><ul>' . $error_message . "</ul>";
                }
            ...
            // pretējā gadījumā veidojam lietotāju
                ...
                try {
                    ...
                    $create_user = Auth::create_user(
                                    Input::post('client_number'), Input::post('password'), Input::post('email'), 1
                    );
                    //Ja nav iespējams izveidot lietotāju, tad iet uz exception bloku
                    if (!$create_user)
                        throw new Exception('Nevarēja izveidot lietotāju!'); // todo
    ...
    public function action_confirm($code = null) {
        ...
        if (Input::method() == 'POST' && Security::check_token()) {
            ...
            if (Input::post('code') == "")
            {
                Session::set_flash('error','Kļūda! Netika ievadīti dati!');
